...
backToTopButton.addEventListener("click", () => {
  ...
  try {
    ...
    const profileUsername = localStorage.getItem("viewUsername");
    ...
    const response = await fetch(`http://localhost:3000/api/users/name/${profileUsername}`, {
      method: "GET",
      headers: {
    ...
    const postsResponse = await fetch(
      `http://localhost:3000/api/posts/user/${data.id}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${jwtToken}`,
    ...
    const followersResponse = await fetch(
      `http://localhost:3000/api/follows/followers/${data.id}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${jwtToken}`,
    ...
    const followingResponse = await fetch(
      `http://localhost:3000/api/follows/following/${data.id}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${jwtToken}`,
